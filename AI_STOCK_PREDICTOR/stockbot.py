...
APIBASEURL = os.getenv('APCA_API_BASE_URL', 'https://paper-api.alpaca.markets')

if APIKEYID and APISECRETKEY:
    ...
    class MockAlpacaAPI:
        def get_account(self):
            class Account:
                ...
                def __str__(self): return "MockAccount(status='ACTIVE')"
            return Account()
        ...
        def get_barset(self, symbol, timeframe, limit):
            class Bar:
                def __init__(self, o, c):
                    ...
                    self.c = c
            import random
            bars = [Bar(random.uniform(50, 100), random.uniform(50, 100)) for _ in range(limit)]
            return {symbol: bars}
        def list_orders(self, status, limit, after):
            return []
    api = MockAlpacaAPI()
    print('WARNING: No API keys found, using mock API')
...
def get_nasdaq_buystocks():
    # Mocking Nasdaq response to bypass 403 blocks
    return {
        'data': {
            'table': {
                'rows': [
...
def get_alpaca_crypto_assets():
    ...
    if r.status_code == 200:
        ...
        print('Error fetching crypto assets: {} - {}'.format(r.status_code, r.text))
        return None
...
Alpaca algo stock trading bot.""".format(STOCKBOT_VERSION)
    ...
    while True:
        try:
            ...
            if (datetime.today().weekday() in BUY_DAYS and datetime.now(tz=TZ).hour == int(get_stocks_h) \
                and datetime.now(tz=TZ).minute == int(get_stocks_m)) or True:
            ...
            if (datetime.today().weekday() in [0,1,2,3,4] and datetime.now(tz=TZ).hour == int(buy_sh) \
                and datetime.now(tz=TZ).minute == int(buy_sm)) or True:
                ...
                while True:
                    ...
                    if len(stock_bought_prices) == MAX_NUM_STOCKS or \
                        equity == 0 or \
                        (datetime.now(tz=TZ).hour == int(buy_eh) and datetime.now(tz=TZ).minute >= int(buy_em)):
                        break
                    else:
                        time.sleep(120)
            ...
            if (datetime.today().weekday() in [0,1,2,3,4] and datetime.now(tz=TZ).hour == int(sell_sh) \
                and datetime.now(tz=TZ).minute >= int(sell_sm)) or True:
                ...
                while True:
                    for stock in bought_stocks:
                        ...
                        stockinfo = [ x for x in stock_bought_prices if x[0] == stock['symbol'] ]
                ...
                if len(stock_sold_prices) < len(bought_stocks) and \
                    ...
                    while True:
                        for stock in bought_stocks:
                            ...
                            if (num_prices >= 15 and went_down > went_up) or (datetime.now(tz=TZ).hour == int(sell_eh) \
                                and datetime.now(tz=TZ).minute >= int(sell_em)):
                                stockinfo = [ x for x in stock_bought_prices if x[0] == stock['symbol'] ]
                                stock_price_buy = stockinfo[0][1]
                                buy_time = stockinfo[0][2]
                                diff = round(stock_price_sell - stock_price_buy, 2)
                        ...
                        if len(stock_sold_prices) == len(bought_stocks) or \
                            (datetime.now(tz=TZ).hour == int(sell_eh) and datetime.now(tz=TZ).minute >= int(sell_em)):
                            break
                        time.sleep(120)
                    ...
                    while True:
                        if datetime.now(tz=TZ).hour == int(sell_eh) and datetime.now(tz=TZ).minute >= int(sell_em) + 5:
                            break
                        time.sleep(60)
